...
module.exports = {
	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand(subcommand => subcommand
			...
			.addStringOption(option => option
				.setName('job')
				.setDescription('Which job would you like:')
				.setRequired(true),
			),
		),
	 ...
	 * @summary Job management - Apply, and quit jobs
	 * 
	 * @param {Object} param
	...
	**/
	execute: async ({ interaction }) => {
		...
		if (subCommandName === 'centre') {
			...
			if (userData.stats.worked < 30) {
				fields = fields.slice(0, 6);
			}
			if (userData.stats.worked < 15) {
			...
			const embed = new EmbedBuilder()
				...
				.addFields(fields)
				.setFooter({
					text: 'Work more to unlock new jobs!',
				});
		...
		if (subCommandName === 'claim') {
			...
			if (userData.stats.job !== 'none' && userData.stats.job !== '') {
				interaction.followUp({
					content: 'You have to leave your current job first!\nUse `/job quit` to leave.',
				});
				return false;
			}
			...
			if (!jobFound || jobFound === undefined || jobFound === null) {
				interaction.followUp({
					content: 'That is not a valid job! Use `/job centre` to view the list',
				});
				return false;
			}
			...
			if (jobFound.req.startsWith('special')) { 
				if (!userData?.achievements?.penPals) {
					conditional = false;
				}
			}
			else {
				...
				if (arg[1] === '>') {
					conditional = (userData[irg[0]][irg[1]] > arg[2]);
				}
			}

			if (userData.stats.worked < requirements[levels.indexOf(jobFound.level)] || !conditional) {
				interaction.followUp({
					content: 'You do not meet the requirements for that job!',
				});
				return false;
			}
			...
			interaction.followUp({
				content: `You became a ${jobFound.emoji} ${jobFound.name}! do\`/work\` to get to work at your new job!`,
			});
		}
