		...
		.addSubcommand(subcommand => subcommand
			...
			.addStringOption(option => option
				...
				.addChoices(
					...
					{ 'name': 'Flipper', 'value': 'flip' },
					{ 'name': 'Avid Flipper', 'value': 'flip_plus' },
					{ 'name': 'Gamer', 'value': 'minigame' },
					{ 'name': 'Pro Gamer', 'value': 'minigame_plus' },
					{ 'name': 'Registered', 'value': 'register' },
					{ 'name': 'Collector', 'value': 'collector' },
					{ 'name': 'Scavenger', 'value': 'collector_plus' },
					{ 'name': 'Wealthy', 'value': 'rich' },
					{ 'name': 'Millionaire', 'value': 'rich_plus' },
				),
			),
		),
	...
	execute: async ({ interaction }) => {
		...
		if (subCommandName === 'claim') {
			...
			if (!badge || !badge.condition) {
				interaction.followUp({
					content: `You can not claim this badge. ${badge.condition}`,
				});
				return false;
			}
			...
			if (userData.badges[badgeId] === true) {
				interaction.followUp({
					content: 'You have already claimed that badge!',
				});
				return false;
			}
			...
			if (type === 'support') {
				if (interaction?.guild?.id !== '821152669141565480') {
					interaction.followUp({
						content: 'This command needs to be ran in the support server.',
					});
					return false;
				}
				allowed = true;
				...
				if (compare === '>') {
					allowed = (ConditionData > Number(value));
				}
				if (compare === '=') {
					allowed = (ConditionData === Number(value));
				}
			...
			if (allowed !== true) {
				interaction.followUp({
					content: `You do not meet the requirements for this badge. ${badge.req}!`,
				});
				return false;
			}

			interaction.followUp({
				content: `You have claimed: ${badge.prof}!`,
			});
