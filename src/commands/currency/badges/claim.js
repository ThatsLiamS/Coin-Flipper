...
module.exports = {
	...
	execute: async function(firestore, args, command, msg, discord, data, send) {
		if (!args[0]) return send("You need to specify which badge to claim! (use `c!badges`)");
		...
		if (!badgeLocalIds.includes(claimed)) return send("That isn't a valid badge (if you're trying to claim the dev or bug hunter badge, use `c!badges` to see how to get them");
		...
		let conditionals = [(msg.guild.id != 832245298578849822), (userData.stats.flipped < 100), (userData.stats.flipped < 300), (userData.stats.minigames_won < 20), (userData.stats.minigames_won < 50), (userData.inv.key < 1), (totalItems < 20), (totalItems < 50), (userData.currencies.cents < 100000), (userData.currencies.cents < 1000000)];
		...
		if (conditional) return send("Sorry, you don't have the requirement to get that badge! (use `c!badges` to see the requirement)");
		let dataBadge = dataIds[badgeLocal.indexOf(badge)];
		let badgeAmt = userData["badges"][dataBadge];
		if (badgeAmt == true) return send("You already have that badge!");
