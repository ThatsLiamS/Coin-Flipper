	...
	execute: async ({ interaction, client }) => {
		...
		if (cmd) {
			const embed = new MessageEmbed()
				...
				.setTimestamp()
				.addField('__Usage:__', '/' + cmd.name + (cmd.usgae ? ' ' + cmd.usage : ''), false);
		...
		const categories = ['flipping', 'currency', 'information', 'karate', 'addons', 'customization', 'online', 'trading'];
		if (categories.includes(cmdName)) {
			...
			if (commandFolders) {
				for (const subCommandFolders of commandFolders) {
					const commandFiles = readdirSync(`${__dirname}/../../commands/${cmdName}/${subCommandFolders}`).filter(file => file.endsWith('.js'));
